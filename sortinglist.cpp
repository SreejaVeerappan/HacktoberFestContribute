#include <iostream>
#include <string>
#include <stdlib.h>

using namespace std;

struct ListNode {
    string name;
    int roll;
    ListNode* next;
};

ListNode* head = NULL;

void append(string name, int roll);
void insert(string name, int roll, bool sortByRoll);
void deleteNode(int roll);
void deleteNodeByName(string name);
void deleteAll();
void printList();
void sorting();
void exitProgram();

int main() {
    int select, roll, num;
    string name;

    while (true) {
        cout << "Press 1 for appending the node" << endl;
        cout << "Press 2 for inserting the node" << endl;
        cout << "Press 3 for deleting the node" << endl;
        cout << "Press 4 for deleting the list" << endl;
        cout << "Press 5 for printing the list" << endl;
        cout << "Press 6 for sorting the list" << endl;
        cout << "Press 7 to exit the program" << endl;
        cin >> select;

        switch (select) {
            case 1:
                cout << "Please enter the name of the student: ";
                cin.ignore(10000, '\n');
                getline(cin, name);
                cout << "Please enter the roll number of the student: ";
                cin >> roll;
                append(name, roll);
                break;
            case 2:
                cout << "Press 1 to sort on the basis of name or 2 to sort on the basis of roll number: ";
                cin >> num;
                cout << "Please enter the name of the student: ";
                cin.ignore(10000, '\n');
                getline(cin, name);
                cout << "Please enter the roll number of the student: ";
                cin >> roll;
                insert(name, roll, num == 1);
                break;
            case 3:
                cout << "Press 1 to delete the node on the basis of name or 2 to delete on the basis of roll number: ";
                cin >> num;
                if (num == 1) {
                    string value;
                    cout << "Please enter the name: ";
                    cin.ignore(10000, '\n');
                    getline(cin, value);
                    deleteNodeByName(value);
                } else if (num == 2) {
                    int value;
                    cout << "Please enter the roll number: ";
                    cin >> value;
                    deleteNode(value);
                } else {
                    cout << "You have entered an invalid input...!" << endl;
                }
                break;
            case 4:
                deleteAll();
                break;
            case 5:
                printList();
                break;
            case 6:
                sorting();
                break;
            case 7:
                exitProgram();
            default:
                cout << "You have entered an invalid input...!" << endl;
                break;
        }
    }
}

void append(string name, int roll) {
    ListNode* temp = new ListNode;
    temp->name = name;
    temp->roll = roll;
    temp->next = NULL;

    if (head == NULL) {
        head = temp;
    } else {
        ListNode* temphead = head;
        while (temphead->next) {
            temphead = temphead->next;
        }
        temphead->next = temp;
    }
}

void insert(string name, int roll, bool sortByRoll) {
    ListNode* temp = new ListNode;
    temp->name = name;
    temp->roll = roll;
    temp->next = NULL;

    if (head == NULL) {
        head = temp;
    } else {
        ListNode* temphead = head;
        ListNode* previous = NULL;

        while (temphead != NULL && (sortByRoll ? roll > temphead->roll : name > temphead->name)) {
            previous = temphead;
            temphead = temphead->next;
        }

        if (previous == NULL) {
            head = temp;
            temp->next = temphead;
        } else if (temphead == NULL) {
            previous->next = temp;
        } else {
            previous->next = temp;
            temp->next = temphead;
        }
    }
}

void deleteNode(int roll) {
    ListNode* temphead = head;
    ListNode* previous = NULL;

    while (temphead != NULL && temphead->roll != roll) {
        previous = temphead;
        temphead = temphead->next;
    }

    if (temphead == NULL) {
        cout << "Sorry no data found!!!" << endl;
    } else if (previous == NULL) {
        head = temphead->next;
        delete temphead;
    } else {
        previous->next = temphead->next;
        delete temphead;
    }
}

void deleteNodeByName(string name) {
    ListNode* temphead = head;
    ListNode* previous = NULL;

    while (temphead != NULL && temphead->name != name) {
        previous = temphead;
        temphead = temphead->next;
    }

    if (temphead == NULL) {
        cout << "Sorry no data found!!!" << endl;
    } else if (previous == NULL) {
        head = temphead->next;
        delete temphead;
    } else {
        previous->next = temphead->next;
        delete temphead;
    }
}

void deleteAll() {
    ListNode* temphead = head;

    while (temphead) {
        ListNode* temp = temphead->next;
        delete temphead;
        temphead = temp;
    }

    head = NULL;
}

void printList() {
    if (head == NULL) {
        cout << "There is nothing to print." << endl;
        return;
    }

    ListNode* temphead = head;
    while (temphead) {
        cout << temphead->name << ' ' << temphead->roll << endl;
        temphead = temphead->next;
    }
}

void sorting() {
    ListNode* temphead = head;
    int counter = 0;

    while (temphead) {
        temphead = temphead->next;
        counter++;
    }

    for (int i = 0; i < counter; i++) {
        for (int j = 0; j < counter - i; j++) {
            if (temphead->roll > temphead->next->roll) {
                swap(temphead->roll, temphead->next->roll);
                swap(temphead->name, temphead->next->name);
            }
            temphead = temphead->next;
        }
        temphead = head;
    }
}

void exitProgram() {
    exit(0);
}
